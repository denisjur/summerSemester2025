...
public abstract class AudioFile {
	...
	protected String pathname; 	// Calea completa catre fisierul audio (zB audiofiles/beethoven.mp3)
	...
	public String toString() {
		...
		if (author == null || author.trim().isEmpty()) {
			return title.trim(); // doar titlu
		}

		return author.trim() + " - " + title.trim(); // format complett
	}

	// Detecteaza daca ruleaza pe Windows
	private boolean isWindows() {
		return System.getProperty("os.name").toLowerCase().indexOf("win") >= 0;
	}

	// Metode abstracte – trebuie implementate in clasele 
